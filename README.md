...
v0.0.19. There is a materials takeoff now, a shopping list built from the active scenario. Click Takeoff and it tallies the outlets by type, the breakers by size and pole, the copper cable by conductor gauge (sized to each circuit's breaker per the NEC, with a fifteen percent slack allowance and a rounded buy figure), the dust duct by diameter with elbow and blast-gate counts, the compressed-air line footage, and the fixed equipment (panels, collector, compressor). Lengths follow the metric or imperial setting, and a Copy button puts a plain-text version on the clipboard. Everything from the earlier releases stands, and the drag and coordinate fixes from v0.0.11 and v0.0.14 hold.
...
Takeoff: the Takeoff button opens a materials list for the active scenario, outlets, breakers, copper cable by gauge, dust duct by diameter (with elbows and blast gates), air-line footage, and equipment, sized from the wired utilities. Wire the shop first (place the nodes and Wire all) so there is something to count. Copy list puts a plain-text version on the clipboard for a purchase order or a note to yourself at the supply house.
