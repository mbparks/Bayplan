...
v0.0.23. Two additions. Notes have a show/hide toggle now: the Notes button in the header hides every plat note at once (and the printed plat and takeoff PDF follow suit), so a drawing can be issued clean or marked up. And light fixtures are a utility: drop a fixture, set its wattage, and Wire all runs it to the nearest panel and groups the fixtures onto shared lighting circuits, distinct from the receptacle and machine circuits, sized by their connected watts. Fixtures show in the load rollup, the per-circuit report, and the takeoff (counted by wattage, with their circuits and cable in the totals), and a fixture with no panel is flagged. Everything from the earlier releases stands, and the drag and coordinate fixes from v0.0.11 and v0.0.14 hold.
