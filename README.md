...
v0.0.8. The full eight-station arc is functional end to end, and this release adds undo/redo across the whole tool. Every edit (placing, dragging, rotating, wiring, shaping the room, adding features, duplicating a scenario) is a step you can walk back and forward with the Undo and Redo buttons or Ctrl/Cmd+Z and Ctrl/Cmd+Shift+Z. A drag is a single step, no-op changes are skipped, and history survives across every mode. Everything from the earlier releases stands: room shaping and features (SOUNDING), placement (STOWAGE), clearance checking with vertical `minHeight` bands (TRIM), material flow (ROUTING), the utilities graph (SERVICES), and scenario-compare with a printable plat (PLAT). All three original data-model open items are closed.
...
The test harness is browser-runnable at `bayplan.html#test`, seeded with the built-in library as its fixture. It covers the geometry core (bounds with offset anchors, edge projection, point-in-polygon, the bearing convention, point-in-arc, the mirror transform, the placement transform, segment intersection, and polygon overlap), the TRIM checker (the full severity-interaction matrix, the vertical-band `minHeight` logic, plus end-to-end scenarios), ROUTING (work-point snapping, travel distance, backtrack detection, and crossing detection), SERVICES (elbow-equivalent inference, dust-run budget warnings, and the electrical rollup with voltage reachability), PLAT (scenario independence after duplication, delta detection, and the printable sheet), SOUNDING (boundary resize and add/remove corner, plus column-obstacle, door-swing, and window conflict behavior), and history (undo, redo, redo-tail truncation on a new edit, no-op dedupe, mid-drag suspension, and library-strip on snapshot). The release gate is green only when every assertion passes.
