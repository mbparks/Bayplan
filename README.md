...
v0.0.13. This release finishes PLAT. Scenario compare now has a picker: when you keep three or more layouts, two dropdowns choose which pair to set side by side, and the plats and delta table update to match. Printing gained real output control: a print setup screen lets you choose paper size (Letter through A3), orientation (or auto, which picks the fewer-page layout), and scale. Fit-to-page issues a single titled sheet as before; a fixed architect's scale (1/2, 1/4, or 1/8 inch to the foot) tiles the plat across as many pages as it needs, each page carrying a caption with the project, scale, and sheet number so the printed pages align by their shared grid. Everything from the earlier releases stands: room shaping with a free-form wall tool and wall-snapping features (SOUNDING), placement (STOWAGE), clearance checking with vertical `minHeight` bands (TRIM), material flow with manual bends and loop-aware backtracks (ROUTING), the utilities graph with hand-routed duct, per-circuit electrical, and a static-pressure dust model (SERVICES), and session undo/redo. All the depth items from the original roadmap are now done.
